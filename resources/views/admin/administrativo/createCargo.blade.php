...
@section('title', isset($cargo) ? 'Edição de Cargo' : 'Criação de Cargo')
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">
                    {{ isset($cargo) ? 'Editando Cargo' : 'Criando Cargos' }}
                </h6>
            </div>
            <!-- Card Body -->
            <div class="card-body">
                <div class="chart-area">
                    <div class="row">
                        <div class="col-md-12">
                            <!-- Form -->
                            <form class="row g-2" action="{{ isset($cargo) ? route('editarCargo.update', $cargo->id) : route('cadastrarCargo.store') }}" method="POST" name="formCadastro">
                                @csrf
                                @if(isset($cargo))
                                    @method('PUT')
                                @endif
                                ...
                                <div class="col-md-12">
                                    <p style="text-align: right;">
                                        <button type="submit" class="btn btn-success text-white" value="salvar">
                                            ...
                                            <span class="text">{{ isset($cargo) ? 'Atualizar' : 'Cadastrar' }}</span>
                                        </button>

                                        @if(isset($cargo))
                                            <a href="{{ route('cadastrarCargo.index') }}" class="btn btn-secondary">
                                            <span class="icon text-white-50">
                            ...
                            @unless(isset($cargo))
                                ...
                                <table class="table table-striped table-md">
                                    <div class="table-responsive">
                                        ...
                                        @foreach ($cargos as $cargoItem)
                                            <tr>
                                                <td class='fw-bold'>{{ $cargoItem->nome }}</td>
                                                <td data-title="Ações" style='text-align:right;'>
                                                    <a href="{{ route('editarCargo.edit', $cargoItem->id) }}">
                                                        <button type='button' class='btn btn-sm btn-warning'>
                                                            <i class="fa-solid fa-pen-to-square"></i>
                                                        </button>
                                                    </a>

                                                    <form action="{{ route('excluirCargo.destroy', $cargoItem->id) }}" method="post"
                                                          style="display:inline-block;">
                                                        @csrf
                                                        @method('delete')
                                ...
                                @if ($cargos->isEmpty())
                                    <p style="text-align: center;">Não existem cargos cadastrados no sistema</p>
                                @endif
                            @endunless
                        </div>
                    </div>
